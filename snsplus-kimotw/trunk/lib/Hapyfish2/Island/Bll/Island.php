...
class Hapyfish2_Island_Bll_Island
{
    ...
    public static function initIsland($ownerUid, $uid, $checkPraise = false, $islandId)
    {
		...
		//奇摩10-14(0点)至10-28(0点) 持续双倍经验
        if (($nowTime >= 1318521601) && $nowTime <= 1319731199) {
			$sumkey = 'i:u:dobuleExp:ev:' . $uid;
			...
			if ($sumint === false) {
	        	...
	        	$doubleexpTime = 1319731199 - $nowTime;

				$userCardStatus['doubleexp'] = $nowTime + $doubleexpTime;

				Hapyfish2_Island_HFC_User::updateCardStatus($uid, $userCardStatus);

				$cache->set($sumkey, 1, 1319731199);
			}
        }
