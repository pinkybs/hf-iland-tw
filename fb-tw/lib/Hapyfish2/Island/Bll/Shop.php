...
/**        //12-6 14:00 至 12-7 14:00 一键收取卡只能购买20张
        $startTime = strtotime('2011-12-06 14:00:00');
        $endTime = strtotime('2011-12-07 14:00:00');
        if (($startTime <= $now) && ($now <= $endTime)) {
        	if (67441 == $itemArray[0]['cid']) {
				if ($itemArray[0]['num'] > 20) {
					$result['content'] = '特賣期間每人只能購買20張一鍵收取卡';
                    return $result;
        		}
        		
        		//一键收取卡购买次数
        		$key = 'ev:card:onekey:' . $uid;
        		$cache = Hapyfish2_Cache_Factory::getMC($uid);
        		$buyNum = $cache->get($key);
        		
        		if (($buyNum + $itemArray[0]['num']) > 20) {
					$result['content'] = '特賣期間每人只能購買20張一鍵收取卡';
                    return $result;
        		}
        	}
...
//				//圣诞节期间统计购买xx
//				if ('' == $background['cid']) {
//					$chrismasTime = strtotime('2011-12-28 23:59:59');
//				    if ($now < $chrismasTime) {
    	...
    	foreach ($buyPlantAry as $plant) {
        	...
        	if ($plant['price_type'] == 1) {
				...
				if ($ok) {
					...
					//买一送一
					$valStartTime = strtotime('2012-02-20 00:00:01');
					$valEndTime = strtotime('2012-02-26 23:59:59');
					
					if (($now >= $valStartTime) && ($now <= $valEndTime)) {
						if (in_array($plant['cid'], array(141732, 141832))) {
							$compensation = new Hapyfish2_Island_Bll_Compensation();
							
							if ($plant['cid'] == 141732) {
								$tpsendCid = 141832;
							} else if ($plant['cid'] == 141832) {
								$tpsendCid = 141732;
							}
								
							$compensation->setItem($tpsendCid, 1);
							$valOK = $compensation->sendOne($uid, '購買“' . $plant['name'] . '”贈送：');
		       ...
		       /** $startTime = strtotime('2011-12-06 14:00:00');
		        ...
		        if (($startTime <= $now) && ($now <= $endTime)) {
		        	if (67441 == $card['cid']) {
						...
						$newCount = $buyNum + $card['count'];
						$cache->set($key, $newCount, 3600 * 24 * 7);
		        	}
		        }*/
			}
