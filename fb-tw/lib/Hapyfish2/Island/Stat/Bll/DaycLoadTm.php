...
class Hapyfish2_Island_Stat_Bll_DaycLoadTm
{
    ...
    public static function calcDayData($dt, $dir)
    {
        ...
        try {
            ...
            //nocookie
            $noFlashData = self::_getNoFlash($dt, $dir);
            $noCookieData = self::_getNoCookie($dt, $dir);
    ...
    private static function _getNoCookie($dt, $dir)
    {
        //$strDate = date('Ymd', $dt);
        $retData = array();
        $strDate = $dt;
        //$fileName = '/home/admin/logs/weibo/stat-data/nocookie/' . $strDate . '/all-nocookie-' . $strDate . '.log';
        $fileName = $dir . $strDate . '/all-nocookie-' . $strDate . '.log';
        try {
            //file not exists
            if (!file_exists($fileName)) {
                info_log($fileName . ' not exists!', 'stat_DaycLoadTm');
    ...
    private static function _getNoFlash($dt, $dir)
    {
        //$strDate = date('Ymd', $dt);
        $retData = array();
        $strDate = $dt;
        $fileName = $dir . $strDate . '/all-noflash-' . $strDate . '.log';
        try {
            //file not exists
            if (!file_exists($fileName)) {
                info_log($fileName . ' not exists!', 'stat_DaycLoadTm');
